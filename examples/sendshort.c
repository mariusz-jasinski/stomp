...
static void _connected(stomp_session_t *s, void *ctx, void *session_ctx)
{
  ...
  int  err = stomp_send(s, sizeof(hdrs)/sizeof(struct stomp_hdr), hdrs, (void *)c->body, strlen(c->body));
  if (err) {
    perror("stomp");
  }  
}
...
static void _receipt(stomp_session_t *s, void *ctx, void *session_ctx)
{
  ...
  const char *id = hdr_get(e->hdrc, e->hdrs, "receipt-id");
  if(id && strcmp(id,"7") == 0)
  {
    ...
    int err = stomp_disconnect(s, sizeof(hdrs3)/sizeof(struct stomp_hdr), hdrs3);
    if (err) {
      perror("stomp");
    }    
  }
  else
  if(id && strcmp(id,"77") == 0)
  {
    stomp_run_off(s);
  } 
}
...
int main(int argc, char *argv[]) 
{
	...
	struct ctx c;
  char host[256]="", port[256]="", paramBody[256] ="", fileName[256]="";  
	stomp_session_t *s;
  int loopnumber = 1;
  ...
  if(isfile){
    c.body = read_body_from_file(fileName);
    if(c.body == NULL){
      exit(EXIT_FAILURE);
    }
  }
